...
/**
 * Alerta exibido no caso do usuario tentar digitar uma quantidade maior de
 * caracteres que o permitido no campo pelo atributo 'maxlength'
 * @param campos Seletor CSS para um ou mais campos
 */
function alertaLimiteSenha(campos){
	var cheio = [];
	$(campos).on(
		'keyup',
		function() {
			var maxLen = $(this).attr("maxlength");
			var alertMaxId = this.id + "Alert";
			var alertMax= $("#" + alertMaxId);
			if (!cheio[this.id]){
				cheio[this.id] = false;
			}

			if (! $("#" + alertMaxId).is("span") ) {
				$(this).after("<span id='" + alertMaxId + "'></span>");
				alertMax = $("#" + alertMaxId);
				alertMax.hide().html("Você atingiu o limite de <strong>" + maxLen + "</strong> caracteres").addClass("help-block");
			}

			if ($(this).val().length == maxLen && cheio[this.id] === true) {
				alertMax.stop(true, true);
				alertMax.fadeIn(500);
				$(this).parents(".form-group").addClass("has-warning");
			}
			else if ($(this).val().length < maxLen) {
				cheio[this.id] = false;
				alertMax.fadeOut(500);
...
function resultadoMatriculaStyle() {
	$("td[data-content-message=matriculaStatus]").each(function(){
		var successExp = /(MATRICULADO|DISCIPLINA CANCELADA COM SUCESSO)/;

		if ( $(this).text().match(successExp) ) {
